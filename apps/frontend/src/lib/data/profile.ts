...
export function registerProfileMutationDefaults(qc: QueryClient) {
  qc.setMutationDefaults(["saveWhatsNew"], {
    ...
    onMutate: async (vars: SaveWhatsNewVars) => {
      // Canonical order (cancel → snapshot → write), matching
      // useSetBabyFeatures above: AuthGate already blocks AppChrome until
      // `me.isFetchedAfterMount`, so there is no one-frame window here for
      // an unrelated in-flight fetch to win. The getting-started redirect's
      // real race is a REMOUNT-triggered fetch, started after this mutation
      // begins — see the `finishingFirstRun` guard in screens/shell.tsx for
      // why that needs its own fix, not a reordering here.
